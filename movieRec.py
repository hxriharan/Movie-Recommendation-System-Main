...
rates = {}
movie_list = []
predict_users = {}                               #Def empty dictionary
...
@app.route("/movierates",methods=['GET', 'POST'])     #Def methods
def getRates():
...
for i in range(1,len(ratings["userId"].unique())+1):      #counting unique elements
...
def fill(row):
    new_row = ((row - row.mean())/(row.max() - row.min()))  #Formula of Normalization
    return new_row
...
def createUser():
    ...
    random.loc[1,:] = 0                                   #2nd row of all columns
    for i in rates.keys():
        random[i] = rates[i]
